...
def registerUser(username, password, name):
	# Make sure no one else is creating this user
	result = transactions.find_and_modify(query = {"username":username}, update = {"$set": {"username":username}}, upsert = True)
	if not result == None and len(result.keys()) > 0: #Note: this return value could vary depending on MongoDB version
		# Someone else is already creating this user
		return False
	elif users.find({"username":username}).count() >=1:
		# User already created
		transactions.remove({"username":username})
		return False
...
def createCalendar(calendarName, calendarOwner, isPrivate=False):
	...
	# Make sure no one else is creating this calendar
	result = transactions.find_and_modify(query = {"calendarname":calendarName, "owner":calendarOwner}, update = {"$set": {"calendarname":calendarName, "owner":calendarOwner}}, upsert = True)
	if not result == None and len(result.keys()) > 0: #Note: this return value could vary depending on MongoDB version
		# Someone else is already creating this calendar
		return False
	...
	# Finished
	transactions.remove({"calendarname":calendarName, "owner":calendarOwner})
...
def editEvent(eventID, field_to_edit, new_contents):
	...
	# Make sure no one else is editing this event
	result = transactions.find_and_modify(query = {"eventID":eventID}, update = {"$set": {"eventID":eventID}}, upsert = True)
	if not result == None and len(result.keys()) > 0: #Note: this return value could vary depending on MongoDB version
		# Someone else is already creating this event
		return False
		...
		for invitee in newEvent["invitees"]:
			...
			for invited_event in invited_user["invited_events"]:
				if invited_event["ID"] == eventID:
					...
					break
	# Finished
	transactions.remove({"eventID":eventID})
